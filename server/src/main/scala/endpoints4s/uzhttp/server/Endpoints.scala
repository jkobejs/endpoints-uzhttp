...
import endpoints4s.{ algebra, Invalid, Valid }
...
trait EndpointsWithCustomErrors extends algebra.EndpointsWithCustomErrors with Requests with Responses {
  ...
  override def choiceRequestEntity[A, B](
    ...
    req =>
      requestEntityA(req).flatMap {
        case Valid(v) => RIO.succeed(Valid(Left[A, B](v)))
        case invalidA =>
          requestEntityB(req).map {
            case Valid(v) => Valid(Right[A, B](v))
            case invalidB => invalidA.zip(invalidB).asInstanceOf[Invalid]
          }
      }
}
